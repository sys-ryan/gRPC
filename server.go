...
func (f *FxGateway) Invoke(c context.Context, s *pb.InvokeServiceRequest) (*pb.Message, error) {
	...
	fmt.Println("-------- Invoke() LOG --------\n")
	ryan_start := time.Now()
	//data, ok := Cache[fmt.Sprintf("%s", s.Service)]
	data, ok := Cache.CacheGet(fmt.Sprintf("%s", s.Service), fmt.Sprintf("%s", s.Input))
	if ok {
		output := data.c_output

		//log
		fmt.Println("[RYAN] Cache HIT")
		fmt.Println(data)
		fmt.Println()
		fmt.Println("output : " )
		fmt.Println(output)
		...
		ryan_end := time.Since(ryan_start)
		fmt.Println("[RYAN] TIME CONSUMED (cache hit) : " + ryan_end.String() + "\n\n")
	...
	//캐싱 안되어있으면 캐싱 수행 (일단 없으면 무조건) 
	/* Cache[fmt.Sprintf("%s", s.Service)] = CachingData{
			serviceName: fmt.Sprintf("%s", s.Service),
			lastCalled: time.Now(),
			c_input: fmt.Sprintf("%s", s.Input),
			c_output: output,
		}
	*/
	Cache.CacheIn(fmt.Sprintf("%s", s.Service), CachingData{
                        serviceName: fmt.Sprintf("%s", s.Service),
                        lastCalled: time.Now(),
                        c_input: fmt.Sprintf("%s", s.Input),
                        c_output: output,
	...
	fmt.Println("[RYAN] Caching COMPLETE !!")
	fmt.Println()

	fmt.Println("[RYAN] s.Service : " + fmt.Sprintf("%s", s.Service))
	fmt.Println("[RYAN] s.Input : " + fmt.Sprintf("%s", s.Input))

	ryan_end := time.Since(ryan_start)
	fmt.Println("[RYAN] TIME CONSUEMED (cache miss) : " + ryan_end.String() + "\n\n")
...
func (f *FxGateway) List(c context.Context, s *pb.Empty) (*pb.Functions, error) {
	...
	fmt.Println("[RYAN] List() called : " +  time.Now().String() + "\n")
...
func (f *FxGateway) Deploy(c context.Context, s *pb.CreateFunctionRequest) (*pb.Message, error) {
	...
	fmt.Println("[RYAN] Deploy() called : " + time.Now().String() + "\n")
